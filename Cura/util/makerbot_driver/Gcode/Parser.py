...
class GcodeParser(object):
    ...
    def __init__(self):
        ...
        self.s3g = None # makerbot_driver.s3gÀà ´´½¨¶ÔÏóºó¸³Öµ
    ...
    def execute_line(self, command):     #½âÊÍÒ»ÐÐµÄGcode
        """
        Execute a line of gcode
        @param string command Gcode command to execute
        """ #ÐèÒª×ª»¯Îªascii¸ñÊ½
        #If command is in unicode, encode it into ascii
        if isinstance(command, unicode):#isinstanceÊÇPythonÖÐµÄÒ»¸öÄÚ½¨º¯Êý,commandÊÇ·ñÊÇunicodeµÄÊµÊµÀý
            self._log.debug('{"event":"encoding_gcode_into_utf8"}')
            command = command.encode("utf8")
        elif not isinstance(command, str):#²»ÊÇasciiÔò±¨´í
