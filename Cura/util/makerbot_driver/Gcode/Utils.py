...
def parse_command(command):
    ...
    pairs = command.split()  #拆分字符串 返回字符数组G162 X Y F2500 ->['G162', 'X', 'Y', 'F2500']
    for pair in pairs:  # pairs = ['G162', 'X', 'Y', 'F2500']
        code = pair[0]
        ...
        # If the code already exists, this is an error. (for example: G0 G0)
        ...
        if len(pair) == 1:  #不带参数的 比如 X Y Z 就算flag
            flags.append(code) #flag 中的值就会是['X','Y']
...
def parse_line(line):
	...
	#对command进行parse 非注释内容 得到code   flag
    return codes, flags, comment


def check_for_extraneous_codes(codes, allowed_codes): #['G','F'],'F'
...
def parse_out_axes(codes): #对codes中的 axes排序 
    ...
    return list(sorted(parsedAxes))  #返回类似['X', 'Y', 'Z']
