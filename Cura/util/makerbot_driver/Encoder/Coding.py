...
def decode_bitfield(number):
    """
    Given an uint8 number 0~255 , returns its decoded bitfield
    ...
    if number < 0 or number > 255:
        raise ValueError
    bitfield = []
    for i in range(8): #i from 0 to 8
        bitfield.append(1 == (number >> i) & 0x01)# 100 --> 1100100B --> [F,F,T,F,F,T,T,F]
    return bitfield #return list of bool type 从低到高
...
def encode_int32(number):
    ...
    """# 将python数据转化成c的字节流
    return struct.pack('<i', number) #python的sign integer转为c的int 小端格式
...
def encode_uint32(number):
    ...
    @return byte array of size 4 that represents the integer
    """
    return struct.pack('<I', number)#python的unsign integer转为c的int 小端格式
...
def encode_axis(axis):
    ...
    return axes_map[axis.lower()] #lower 应该是字符换成小写 Gcode中有大写
...
def unpack_response(format, data):
    ...
    try:
        return struct.unpack(format, buffer(data))#将收到的字节流按一定格式顺序解析
