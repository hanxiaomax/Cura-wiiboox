	...
	def validate(self):
		try:
			...
			if self.minValue is not None and f < self.minValue:
				return ERROR, _('This setting should not be below ') + str(round(self.minValue, 3))
			if self.maxValue is not None and f > self.maxValue:
				return ERROR, _('This setting should not be above ') + str(self.maxValue)
			return SUCCESS, ''
		except (ValueError, SyntaxError, TypeError, NameError):
			return ERROR, '"' + str(self.setting.getValue()) + _('" is not a valid number or expression')
...
class validInt(object):
	...
	def validate(self):
		try:
			...
			if self.minValue is not None and f < self.minValue:
				return ERROR, _('This setting should not be below ') + str(self.minValue)
			if self.maxValue is not None and f > self.maxValue:
				return ERROR, _('This setting should not be above ') + str(self.maxValue)
			return SUCCESS, ''
		except (ValueError, SyntaxError, TypeError, NameError):
			return ERROR, '"' + str(self.setting.getValue()) + _('" is not a valid whole number or expression')
	...
	def validate(self):
		...
		try:
			...
			if wallThickness <= nozzleSize * 0.5:
				return ERROR, _('Trying to print walls thinner then the half of your nozzle size, this will not produce anything usable')
			if wallThickness <= nozzleSize * 0.85:
				return WARNING, _('Trying to print walls thinner then the 0.8 * nozzle size. Small chance that this will produce usable results')
			if wallThickness < nozzleSize:
				return SUCCESS, ''
			if nozzleSize <= 0:
				return ERROR, _('Incorrect nozzle size')
			
			lineCount = int(wallThickness / nozzleSize)
			lineWidth = wallThickness / lineCount
			lineWidthAlt = wallThickness / (lineCount + 1)
			if lineWidth >= nozzleSize * 1.5 and lineWidthAlt <= nozzleSize * 0.85:
				return WARNING, _('Current selected wall thickness results in a line thickness of ') + str(lineWidthAlt) + _('mm which is not recommended with your nozzle of ') + str(nozzleSize) + 'mm'
			return SUCCESS, ''
		except ValueError:
			#We already have an error by the int/float validator in this case.
			return SUCCESS, ''
	...
	def validate(self):
		...
		try:
			...
			if printVolumePerSecond > maxPrintVolumePerSecond:
				return WARNING, _('You are trying to print more then %.1fmm^3 of filament per second. This might cause filament slipping. (You are printing at %0.1fmm^3 per second)') % (maxPrintVolumePerSecond, printVolumePerSecond)
